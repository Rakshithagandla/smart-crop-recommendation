...
from models import db, User, Farmer, Recommendation, AuditLog, Feedback
...
FERTILIZER_MAP = {
    ...
    'mango':'NPK 13-00-45 + Micronutrients', 'banana':'NPK 13-00-45 + Magnesium',
    'grapes':'NPK 0-52-34 + Calcium Nitrate', 'default':'NPK 17-17-17 (Balanced)'
}
...
# ---- FARMER AUTH (Password-based, no OTP) ----
...
def farmer_register():
    ...
    if not all([name, aadhar, password]):
        return jsonify({'success': False, 'error': 'Name, Aadhaar and password are required'}), 400
    ...
    user = User(email=f"farmer_{aadhar}@smartcrop.local", phone='', name=name,
    ...
    return jsonify({'success': True, 'token': token,
                    'user': {'id': user.id, 'name': user.name, 'role': user.role, 'farmer_id': farmer.id}}), 201
...
def farmer_login():
    ...
    return jsonify({'success': True, 'token': token,
                    'user': {'id': user.id, 'name': user.name, 'role': user.role,
                             'farmer_id': farmer.id if farmer else user.id}}), 200
...
def farmer_forgot_password():
    ...
    if not aadhar or not new_password:
    ...
    db.session.commit()
    return jsonify({'success': True, 'message': 'Password updated successfully! Please login.'}), 200
...
def officer_register():
    ...
    password        = d.get('password','')
    officer_id_code = d.get('officer_code','').strip()
    ...
    if officer_id_code != OFFICER_REG_CODE:
        return jsonify({'success': False, 'error': f'Invalid officer registration code. Ask your department for the code.'}), 403
    ...
    token = make_token(user)
    return jsonify({'success': True, 'token': token,
                    'user': {'id': user.id, 'name': user.name, 'role': user.role}}), 201
...
def add_farmer(current_user_id):
    ...
    if not all([name, aadhar]):
        return jsonify({'success': False, 'error': 'Name and Aadhaar are required'}), 400
    if len(aadhar) != 12 or not aadhar.isdigit():
    ...
    farmer = Farmer(name=name, aadhar=aadhar, phone=phone,
                    literacy_status='illiterate', officer_id=current_user_id)
    db.session.add(farmer)
    db.session.commit()
    return jsonify({'success': True, 'message': f'{name} added successfully',
                    'farmer': {'id': farmer.id, 'name': farmer.name, 'phone': farmer.phone}}), 201
        ...
        try:
            ...
            if fid:
                db.session.add(Recommendation(
                    ...
                    fertilizer=fertilizer, confidence=confidence, weather_data=weather
                ))
                db.session.commit()
        except Exception as e:
            print(f"⚠ DB save: {e}")
...
def submit_feedback(current_user_id):
    ...
    if not crop or rating is None:
        return jsonify({'success': False, 'error': 'Crop and rating are required'}), 400
    ...
    db.session.add(Feedback(
        user_id=current_user_id, farmer_id=farmer.id if farmer else None,
...
def get_feedback():
    ...
    all_ratings = [f.rating for f in all_q.all()]
    ...
    return jsonify({
        ...
        'total_reviews': len(all_ratings), 'rating_dist': dist,
        'feedbacks': [{
            'id': f.id,
            'farmer_name': f.user.name if f.user else 'Anonymous',
            'crop': f.crop, 'rating': f.rating, 'comment': f.comment,
            'location': f.location, 'season': f.season,
